...
public class LearningServiceImpl implements LearningService {
    ...
    public RestResponse<String> getVideo(String userId, Long courseId, Long teachplanId, String mediaId) {
        ...
        if (teachplan == null) {
            return RestResponse.validfail("教学计划信息不存在");
        }
        ...
        if (StringUtil.isNotEmpty(userId)){
            // 3.1 判断是否选课
            XcCourseTablesDto courseTable = courseTablesService.getLearningStatus(userId, courseId);
            String learnStatus = courseTable.getLearnStatus();
